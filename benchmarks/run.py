...
def main(argv: list[str] | None = None) -> int:
    from benchmarks.cases import benchmark_environment_metadata, iter_cases

    parsed, pyperf_argv = _parse_args(argv or sys.argv[1:])
    sys.argv = [sys.argv[0], *pyperf_argv]
    groups, match = _resolve_selection(parsed)

    cases = iter_cases(match=match, groups=groups)
    if not cases:
