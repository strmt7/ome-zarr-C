...
bool square_less_than(const std::int64_t value, const std::int64_t threshold) {
    if (threshold <= 0) {
        return false;
    }
    if (value == 0) {
    ...
    return value <= (threshold - 1) / value;
}

std::int64_t largest_nonnegative_square_less_than(const std::int64_t threshold) {
    auto candidate = static_cast<std::int64_t>(
        std::sqrt(static_cast<long double>(threshold - 1)));
    while (square_less_than(candidate + 1, threshold)) {
        ++candidate;
    }
    while (!square_less_than(candidate, threshold)) {
        --candidate;
    }
    return candidate;
}
...
std::vector<CirclePoint> circle_points(std::size_t height, std::size_t width) {
    ...
    const bool use_span_fill = radius > 16U;
    ...
    for (std::size_t y = 0; y < height; ++y) {
        ...
        const auto max_dx =
            largest_nonnegative_square_less_than(signed_radius_squared - dy_squared);
        const auto first_x = signed_cx > max_dx ? signed_cx - max_dx : 0;
        const auto last_x = std::min<std::int64_t>(
            static_cast<std::int64_t>(width - 1),
            signed_cx + max_dx);
        for (auto x = first_x; x <= last_x; ++x) {
            points.push_back(CirclePoint{y, static_cast<std::size_t>(x)});
        }
    }
    return points;
}
