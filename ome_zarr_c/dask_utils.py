"""Dask utility helpers kept behavior-compatible with ome-zarr-py v0.15.0."""
...
def zoom(
    ...
    output_shape = tuple(d // f for d, f in zip(image.shape, factors, strict=False))
    output_slices = tuple(slice(0, d) for d in output_shape)
    output = da.map_blocks(
...
def downscale_nearest(image: da.Array, factors: tuple[int, ...]) -> da.Array:
    ...
    if not (
        all(
            isinstance(f, int) and 0 < f <= d
            for f, d in zip(factors, image.shape, strict=False)
        )
    ):
        raise ValueError(
